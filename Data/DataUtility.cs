    ...
    public class DataUtility
    {
        ...
        public static async Task SeedDefaultNotificationTypesAsync(ApplicationDbContext context)
        {
            try
            {
                ...
                var dbNotificationTypes = context.NotificationsTypes.Select(c => c.Name).ToList();
                await context.NotificationsTypes.AddRangeAsync(notificationTypes.Where(c => !dbNotificationTypes.Contains(c.Name)));
                await context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
